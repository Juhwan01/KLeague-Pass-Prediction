...
PATH_TEST_INDEX = f"{BASE_PATH}test.csv"
PATH_MATCH_INFO = f"{BASE_PATH}match_info.csv"
PATH_SAMPLE_SUB = f"{BASE_PATH}sample_submission.csv"
...
for _, row in test_index.iterrows():
    df_ep = pd.read_csv(BASE_PATH+row["path"])
...
def assign_pos_in_K(group):
    group = group.sort_values("event_idx")  # 오래된 → 최근
    L = len(group)
    group["pos_in_K"] = np.arange(K - L, K)
    return group

# groupby 후 game_episode가 유지되도록 처리
lastK = lastK.groupby("game_episode", group_keys=False, as_index=False).apply(assign_pos_in_K).reset_index(drop=True)
